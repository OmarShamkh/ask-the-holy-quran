...
def get_pooling_results(query_tokens, model_vectors, method):
  ...
  for verse in quran_clean_text:
    ...
    cosine_similarity = np.dot(query_method_pooling_vec, verse_method_pooling_vec) / (np.linalg.norm(query_method_pooling_vec) * np.linalg.norm(verse_method_pooling_vec))
